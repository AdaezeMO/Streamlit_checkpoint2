...
country_encoder = {'Kenya' : 0,
  'Rwanda' : 1,
  'Tanzania' : 2,
  'Uganda' : 3}

location_type_encoder = {'Rural' : 0,
  'Urban' : 1}

cellphone_access_encoder = {'No' : 0,
  'Yes' : 1}

gender_of_respondent_encoder = {'Female' : 0,
  'Male' : 1}

relationship_with_head_encoder = {'Child' : 0,
  'Head of Household' : 1,
...
marital_status_encoder = {'Divorced/Seperated' : 0,
  'Dont know' : 1,
  'Married/Living together' : 2,
  'Single/Never Married' : 3,
  'Widowed' : 4}

education_level_encoder = {'No formal education' : 0,
...
with col2:
    ...
    relationship_with_head = relationship_with_head_encoder[st.selectbox("relationship_with_head", df["relationship_with_head"].unique())]
    marital_status = marital_status_encoder[st.selectbox("marital_status", df["marital_status"].unique())]
    education_level = education_level_encoder[st.selectbox("education_level", df["education_level"].unique())]
    job_type = job_type_encoder[st.selectbox("job_type", df["job_type"].unique())]

#how to create a "country dropdown"
...
input_data = pd.DataFrame({
    ...
    "gender_of_respondent": age_of_respondent,
